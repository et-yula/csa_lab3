...
def translate(tokens: list[str], ast: AST) -> str:
    ...
    def t_is(token: str, type: str):
        ...
        return ((type=="variable" and re.match(r'^[A-Za-z][A-Za-z0-9]*$', token))or
            (type=="number" and re.match(r'^(0|-?[1-9][0-9]*)$', token))or
            (type=="string" and re.match(r'^("[^"]*"|\'[^\']\')$', token)))
    ...
    def set_varible(name: str, scope: dict[str, (str, int)], is_array=False) -> list[str]:
        return [{"instruction":"LD","operand":"SP+0"}, {"instruction":"ST","operand":"["+str(scope[name][1])+"]"}] if is_array else [{"instruction":"LD","operand":"SP+0"}, {"instruction":"ST","operand":str(scope[name][1])}]
    ...
    def compile_str(name: str, scope: dict[str, (str, int)]) -> list[str]: # string, varible, number
        if t_is(name, "variable"):
            ...
            if scope[name][0] == "variable":
                return [{"instruction":"LD","operand":"["+str(scope[name][1])+"]"}, {"instruction":"PUSH"}]
        ...
        if t_is(name, "number"):
            return [{"instruction":"LD","operand":str(name)}, {"instruction":"PUSH"}]
    ...
    def compile(ast: AST, scope: dict[str, (str, int)]):
        ...
        if isinstance(ast.args[0], AST) or t_is(ast.args[0], "string"): # ((code) (code) (code) ...)
            ...
            t_assert(not isinstance(ast.args[1], AST) and t_is(ast.args[1], "variable"), "setq expects a variable as the first argument", ast)
            if ast.args[0] == "defvar":
                t_define(ast.args[1], "variable", scope)
            t_assert(ast.args[1] in scope and scope[ast.args[1]][0]=="variable", ast.args[1]+" is not variable", ast)
            ...
            t_assert(not isinstance(ast.args[1], AST) and t_is(ast.args[1], "number") and int(ast.args[1])>0, ast.args[0]+" expects a number as the first argument", ast)
            ast.code.extend([{"instruction":"LD", "operand": str(len(global_data))}, {"instruction":"PUSH"}])
            ...
            if isinstance(ast.args[1], AST):
                ...
                ast.code.extend(compile_str(ast.args[1], scope))
            if ast.args[0] == "getv":
                ast.code.extend([{"instruction":"LD", "operand": "[SP+0]"}, {"instruction":"ST", "operand": "SP+0"}])
            else:
            ...
            if ast.args[0] in ("+", "*"):
                ast.code.append({"instruction":"POP"})
                ast.code.append({"instruction":({"+": "ADD", "*": "MUL"}[ast.args[0]]), "operand": "[SP+0]"})
                ast.code.append({"instruction":"ST", "operand": "SP+0"})
            if ast.args[0] in ("-", "/", "%"):
                ast.code.append({"instruction":"LD", "operand": "SP+1"})
                ast.code.append({"instruction":({"-": "SUB", "/": "DIV", "%": "MOD"}[ast.args[0]]), "operand": "[SP+0]"})
            ...
            if ast.args[0] in ("=", ">=", "!="):
                ...
                ast.code.append({"instruction":({"=": "JE", "!=": "JNE", ">=": "JGE"}[ast.args[0]]), "V": lable1})
            ...
            t_assert(len(ast.args)>3, ast.args[0]+" expects more 3 arguments (name, arguments, ...body)", ast)
            t_assert(not isinstance(ast.args[1], AST) and t_is(ast.args[1], "variable"), "defun expects a name as the first argument", ast)
            t_assert(isinstance(ast.args[2], AST) and all([not isinstance(arg, AST) and t_is(arg, "variable") for arg in ast.args[2].args]), "defun expects a arguments as the second argument", ast)
            ...
            for i in range(len(ast.args[2].args)):
                ...
                ast.code.append({"instruction":"LD", "operand": "SP+"+str(len(ast.args[2].args)-i)})
                ast.code.append({"instruction":"ST", "operand": str(fscope[ast.args[2].args[i]][1])})
            ...
            t_assert(len(ast.args)>2, ast.args[0]+" expects more 2 arguments (condition, ...body)", ast)
            if ast.args[0] == "while":
                lable1 = rnd_lable();
                lable2 = rnd_lable();
            ...
            t_assert(scope[ast.args[0]][0]=="function", ast.args[0]+ " is not function", ast)
            for arg in ast.args[1:]:
                if isinstance(arg, AST):
                    compile(arg, scope);
                    ast.code.extend(arg.code)
